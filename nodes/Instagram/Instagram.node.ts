			...
			},
			{
				...
				options: [
					...
					},
					{
						...
						default: '',
						description:
							'Custom name for the original audio associated with this reel. Applies only to Reels.',
					},
					{
						...
						default: '',
						description:
							'Comma-separated Instagram usernames to invite as collaborators on this reel (without @). Applies only to Reels.',
					},
					{
						...
						default: '',
						description:
							'URL of an image to use as the custom cover for the reel. Applies only to Reels.',
					},
					{
					...
					},
					{
						...
						default: true,
						description:
							'Whether to share the reel to the Instagram feed as well as the Reels tab. Applies only to Reels.',
					},
					{
						...
						default: 0,
						description:
							'Time in seconds from the start of the video to use for the reel thumbnail. Applies only to Reels.',
					},
					{
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
			try {
				...
				if (resource === 'reels') {
					const shareToFeed = additionalFields.shareToFeed as boolean | undefined;
					if (shareToFeed !== undefined) {
						mediaQs.share_to_feed = shareToFeed;
					}

					const collaboratorsRaw = additionalFields.collaborators as string | undefined;
					const collaborators = collaboratorsRaw?.trim();
					if (collaborators) {
						mediaQs.collaborators = collaborators;
					}

					const coverUrlRaw = additionalFields.coverUrl as string | undefined;
					const coverUrl = coverUrlRaw?.trim();
					if (coverUrl) {
						mediaQs.cover_url = coverUrl;
					}

					const audioNameRaw = additionalFields.audioName as string | undefined;
					const audioName = audioNameRaw?.trim();
					if (audioName) {
						mediaQs.audio_name = audioName;
					}

					const thumbOffset = additionalFields.thumbOffset as number | undefined;
